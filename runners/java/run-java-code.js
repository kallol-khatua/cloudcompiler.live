...
const runJavaCode = (file_name, job_id, options = {}) => {
    return new Promise(async (resolve, reject) => {
        ...
        // Java command with memory limit
        const javaCmd = `java -Xmx${config.memoryLimit} ${file_name}`;

        const ptyProcess = pty.spawn("bash", ["-c", `stty -echo; ${javaCmd}`], {
            name: "dumb",
            cols: 100,
            rows: 30,
            cwd: jobsDir,
        ...
        ptyProcess.onExit(({ exitCode, signal }) => {
            ...
            if (exitCode === 0) {
                console.log("Java code run successfully");
                ...
                resolve({
                    job_id,
                    message: "Java code run successfully",
                    metrics: {
                        executionTime,
                        // outputSize,
                        outputLines
